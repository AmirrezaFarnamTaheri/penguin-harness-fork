...
1. **Announce the scan plan** in one short paragraph: what dirs/files you will
   ...
   > Ready to scan manifest files (`package.json`, `pyproject.toml`, `go.mod`) +
   > project structure + existing configs to recommend hooks,
   > subagents, skills, and MCP tools (~30K tokens estimated).
