...
5. As the final tool call of the delivery turn, call WorkBuddy's `present_files` (the "send file" tool) with the local `file://` URI first, the task directory as `cwd`, and a short explanation. This lets WorkBuddy open the exported media in the system/default player.
