...
Zone B: Example/placeholder body content ("1chapter XXX", sample paragraphs)
...
for i, element in enumerate(template_body_elements):
    ...
    if style in heading1_styles and ("1chapter" in text or "Chapter 1" in text):
        replace_start = i
...
**KEEP in `<w:rPr>`:**
- `<w:rFonts w:eastAsia="SimSun"/>` — CJK font declaration (MUST keep, or Chinese text renders wrong)
- `<w:rFonts w:eastAsia="STZhongsong"/>` — same reason
