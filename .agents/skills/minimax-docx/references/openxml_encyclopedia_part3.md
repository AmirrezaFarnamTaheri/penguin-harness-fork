    ...
    int? existingCommentId = null)
{
    ...
    var commentPara = new Paragraph(
        new ParagraphProperties(
            ...
            newrare archaic charactersasiId { Val = paraId }
        ),
        new Run(
...
public static void ResolveComment(WordprocessingDocument doc, int commentId)
{
    ...
    if (comment != null)
    {
        ...
        var paraIdElem = para?.ParagraphProperties?
            .Elements<rare archaic charactersasiId>().FirstOrDefault();
...
new M.Fraction(
    ...
    new M.Denominator(...),
    new M.FractionPr(
        new M.Type { Val = M.FractionValues.Skewed }  // or Normal, Linear,silk
    )
);
```
...
```words,link numbering to heading styles so that Heading1 starts a new numbering sequence, Heading2 is a sub-item, etc.
...
NumberFormatValues.Kanji           // Kanji numerals
NumberFormatValues.KanjiDigit      // 1, 2, 3...
NumberFormatValues.DoubleByte      // Ideographic: 1, 2, 3
