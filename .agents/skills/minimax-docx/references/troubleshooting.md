...
**Symptom:** Template specifies SimSun/Times New Roman but document shows Google Sans, Arial, Calibri, or whatever font the source document used.
...
**Symptom:** Table of contents shows the template's example entries (e.g., "1chapter introduction...1") instead of actual headings. Or clicking "Update Table" in Word does nothing.
...
foreach (var run in body.Descendants<Run>())
{
    ...
    if (ContainsCjk(text))
    {
        ...
        rFonts.EastAsia = "SimSun"; // SimSun — safest default
...
Common safe CJK fonts: SimSun (SimSun), SimHei (SimHei), FangSong (FangSong), KaiTi (KaiTi).
