...
|--------|----------|----------------|-----------|
| `list`, `catalog`, `show all` | Catalog mode (cache bypass) | Category-grouped skill list | `reference/catalog.md` |
| `which one`, `recommend`, `suggest` | Matching mode (cache-first) | 1-3 recommendations + rationale | `.claude/compass-cache.md` OR `reference/patterns.md` |
| `difference`, `compare`, `A vs B` | Comparison mode | Diff table + usage guide | `reference/catalog.md` |
| `first time`, `onboarding`, `how to use` | Onboarding mode | Step-by-step guide | — |
| `combination`, `chain`, `workflow` | Chain mode | Agent chain proposal | `reference/patterns.md` |
| `create cache`, `init`, `speedup` | Cache init mode | Cache file + 5-line report | `reference/cache-recipes.md` |
| `update cache`, `refresh`, `regenerate` | Cache refresh mode | Cache file + before/after diff | `reference/cache-recipes.md` |
...
|--------|----------------------|---------------|
| **Build** | Builder, Forge, Artisan | `/builder implement user authentication API` |
| **Fix** | Scout, Zen, Bolt | `/scout getting 500 error on login` |
| **Guard** | Sentinel, Radar, Judge | `/radar add tests for this module` |
| **Design** | Atlas, Schema, Gateway | `/atlas analyze dependencies` |
| **Operate** | Gear[gha], Scaffold, Beacon | `/gear gha create GitHub Actions workflow` |
