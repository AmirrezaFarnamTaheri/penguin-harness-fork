...
- regulated_breach_notification_routing: Routes a `data_breach`-classified incident to `canon` for jurisdiction-aware disclosure (GDPR 72h notification, HIPAA Breach Notification Rule, Act on the Protection of Personal Information (APPI), CCPA, EU NIS2). Pattern G: `triage` detects → `canon` drafts disclosure copy → `canon[regulatory]` validates per G14 → back to `triage` for IC sign-off.
