...
describe("RepeatToolGuard runtime integration", () => {
  /** Fake environment: closes each tool call with a fixed complete output (no real shell). */
  function repeatEnvironment(): EnvironmentInterface {
    return {
      listTools: async () => [],
      toolPermission: (): ToolPermission => "rw",
      async *executeTool({ toolCall: tc }) {
        yield toolCallOutput({ output: "result", toolCallId: tc.payload.tool_call_id });
      },
    };
  }

  const inputTexts = (msgs: OmniMessage[]): string[] =>
    msgs.map((m) => {
      const p = m.payload as { text?: string; output?: string };
      return p.text ?? p.output ?? "";
    });

  it("delivers the repeat reminder on the stream and in the next turn's input after three identical calls", async () => {
    /** Issues three byte-identical tool calls in one response, then finishes. */
    class RepeatCallLLM implements LLMInterface {
      calls = 0;
      inputs: OmniMessage[][] = [];
      async *streamGenerate(
        params: GenerativeModelParameters,
        ...
        if (this.calls === 1) {
          // The guard's chain climbs 1 -> 2 -> 3 across the identical calls and fires the gentle
          // reminder on the third — all within this one response.
          for (let i = 1; i <= 3; i += 1) {
            yield toolCall({
              name: "search",
              arguments: JSON.stringify({ q: "penguin" }),
              toolCallId: `call_${i}`,
              stopReason: "completed",
            });
          }
        ...
        yield assistantText("Switching approach after the reminder.");
        yield tokenUsage(emptyTokenCounts(), {
          cache_read: 0,
          cache_write: 0,
          output: 8,
    ...
    const llm = new RepeatCallLLM();
    const engine = new ContextEngine({ llm, environment: repeatEnvironment() });
    const all = await collectRun(engine, [userText("find penguin facts")], allowAll);

    expect(llm.calls).toBe(2);

    // The reminder rode the output stream as a user-role message.
    const streamed = all
      .map((m) => m.payload as { role?: string; text?: string })
      .filter((p) => p.role === "user" && (p.text ?? "").includes("[repeat_tool_reminder]"));
    expect(streamed).toHaveLength(1);
    expect(streamed[0]!.text).toContain("repeating the exact same tool call");

    // The next turn's input carried the reminder after the three tool outputs
    // (tool_use/tool_result pairing intact, reminder appended last).
    const texts = inputTexts(llm.inputs[1]!);
    expect(texts.filter((t) => t === "result")).toHaveLength(3);
    expect(texts[3]).toContain("[repeat_tool_reminder]");
  });

  it("does not remind when consecutive calls differ", async () => {
    /** Issues three tool calls with different arguments, then finishes. */
    class VaryingLLM implements LLMInterface {
      async *streamGenerate(
        ...
        if (this.calls <= 3) {
          yield toolCall({
            name: "search",
            arguments: JSON.stringify({ q: `penguin-${this.calls}` }),
    ...
    const engine = new ContextEngine({ llm: new VaryingLLM(), environment: repeatEnvironment() });
    const all = await collectRun(engine, [userText("go")], allowAll);
    expect(
      all.some((m) =>
        ((m.payload as { text?: string }).text ?? "").includes("[repeat_tool_reminder]"),
      ),
    ).toBe(false);
  });

  it("escalates across turns: the gentle reminder follows call 3, the detailed one follows call 5", async () => {
    /** One byte-identical call per turn for five turns, recording every request's input. */
    class LoopLLM implements LLMInterface {
      calls = 0;
      inputs: OmniMessage[][] = [];
      async *streamGenerate(
        params: GenerativeModelParameters,
    ...
    const all = await collectRun(
      new ContextEngine({ llm, environment: repeatEnvironment() }),
      [userText("go")],
      allowAll,
    );

    expect(llm.calls).toBe(6);
    // Each reminder rides the request AFTER the call that triggered it: the gentle one past call
    // 3, the escalating detailed one past call 5.
    const afterThree = inputTexts(llm.inputs[3]!);
    expect(afterThree[afterThree.length - 1]).toContain("[repeat_tool_reminder]");
    expect(afterThree[afterThree.length - 1]).toContain("repeating the exact same tool call");
    const afterFive = inputTexts(llm.inputs[5]!);
    expect(afterFive[afterFive.length - 1]).toContain("Repeated tool call detected:");
    expect(afterFive[afterFive.length - 1]).toContain("consecutive_calls: 5");
    // The request that produced the third call carried no reminder yet.
    expect(inputTexts(llm.inputs[2]!).some((t) => t.includes("[repeat_tool_reminder]"))).toBe(
      false,
    );
    // Exactly two reminders on the stream, one per threshold reached.
    expect(
      all.filter((m) =>
        ((m.payload as { text?: string }).text ?? "").includes("[repeat_tool_reminder]"),
      ),
  ...
  it("emits no reminders at all when the host disables the guard with null", async () => {
    /** Five identical calls; the engine is constructed with the guard explicitly disabled. */
    class LoopLLM implements LLMInterface {
      calls = 0;
