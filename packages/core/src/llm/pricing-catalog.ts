 ...
 * Absorbed and unified from 9router, tokscale, token-monitor, and gpt-load.
 */
...
export const DEFAULT_PRICING_CATALOG: readonly ModelPricingEntry[] = [
  ...
  },
  {
    provider: "qwen",
    modelId: "qwen-max",
    promptPerMillion: 1.6,
    completionPerMillion: 6.4,
  },
  // Kimi / Moonshot
  {
    provider: "kimi",
  ...
  },
  {
    provider: "kimi",
    modelId: "kimi-k1.5",
    promptPerMillion: 1.5,
    completionPerMillion: 4.5,
  },
  // xAI Grok
  {
    provider: "xai",
...
export class PricingCatalog {
  ...
  constructor(customEntries?: ModelPricingEntry[]) {
    if (customEntries) {
      for (const e of customEntries) {
        this.customPricing.set(`${e.provider}:${e.modelId}`.toLowerCase(), e);
      }
    }
  }
  ...
  resolve(provider: string, modelId: string): ModelPricingEntry | undefined {
    ...
    // Match default catalog
    const exact = DEFAULT_PRICING_CATALOG.find(
      (e) => e.provider.toLowerCase() === provider.toLowerCase() && e.modelId.toLowerCase() === modelId.toLowerCase(),
    );
    if (exact) return exact;

    // Fuzzy match on modelId substring (e.g. claude-3-7-sonnet vs claude-3-7-sonnet-20250219)
    const fuzzy = DEFAULT_PRICING_CATALOG.find(
      (e) =>
        e.provider.toLowerCase() === provider.toLowerCase() &&
        (modelId.toLowerCase().includes(e.modelId.toLowerCase()) || e.modelId.toLowerCase().includes(modelId.toLowerCase())),
    );
    return fuzzy;
  }
  ...
  calculateCost(
    ...
    // Theoretical prompt cost without caching
    const fullUncachedCost = ((promptTokens + cacheReadTokens) / 1_000_000) * pricing.promptPerMillion;
    const actualPromptAndCacheCost = promptCost + cacheReadCost;
    const savingsFromCache = Math.max(0, fullUncachedCost - actualPromptAndCacheCost);
