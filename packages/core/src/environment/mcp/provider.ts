 ...
 * private catalog on `tools/list_changed`; its two model-visible schemas never change. When a
 * compaction opens the next model context, `reconfigure` keeps every connection whose entry is
 * unchanged (no respawn, no re-discovery), closes the removed or changed ones, and leaves the new
 * or changed ones pending for the next phase.
 *
...
export class McpToolProvider {
  ...
  /** Connects the pending servers and returns the model-visible tool surface for this Session. */
  ...
  private async connectAll(): Promise<void> {
    ...
    for (const { conn, result } of settled) {
      ...
      for (const tool of conn.tools) {
        ...
        if (this.byName.has(name) || this.targetByName.has(name)) registered += 1;
      }
      result.tools = registered;
      if (conn.refreshRequested) this.queueCatalogRefresh(conn);
    }
    if (this.exposure === "auto" && this.autoUsesGateway === null) {
  ...
  private rebuildRegistry(): void {
    ...
    if (this.exposure === "lazy") {
      ...
      this.registerStaticCatalogTools();
    }
    ...
    for (const server of this.servers) {
      ...
      for (const tool of conn.tools) {
        this.register(conn, tool, seen);
      }
      conn.catalogReady = true;
    }
    if (this.exposure === "auto" && this.autoUsesGateway === true) {
      this.activateAutoGateway();
    }
  }
  ...
  /** Returns whether the tool joined the toolset (skips are warned, never thrown). `seen` holds the names already taken on this server. */
  private register(conn: McpConnection, tool: Tool, seen?: Set<string>): boolean {
    const name = mcpToolName(conn.server.name, tool.name);
    ...
    if (seen ? seen.has(name) : this.byName.has(name)) {
      this.warn(
    ...
    const wrapper: BuiltinTool = {
      ...
      execute: async function* (args, ctx): AsyncGenerator<OmniMessage, ToolResult> {
        ...
        try {
          ...
          return { stopReason: retryable ? "retryable" : "fatal" };
        }
