...
export function DiffBlock({
  ...
  const handleCopy = () => {
    const raw = lines
      .map((l) => (l.type === "add" ? `+ ${l.content}` : l.type === "remove" ? `- ${l.content}` : `  ${l.content}`))
      .join("\n");
        ...
        {visibleLines.map((line, idx) => {
          if (line.type === "add") {
            return (
              ...
              >
                <span className="select-none font-bold text-emerald-600 dark:text-emerald-400 shrink-0">+</span>
          ...
          if (line.type === "remove") {
            return (
              ...
              >
                <span className="select-none font-bold text-rose-600 dark:text-rose-400 shrink-0">-</span>
                <pre className="whitespace-pre-wrap break-all font-mono">{line.content}</pre>
              </div>
            );
          }
