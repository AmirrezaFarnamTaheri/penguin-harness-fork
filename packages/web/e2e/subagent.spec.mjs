...
test("subagent renders as a chip; the panel shows the call graph and child conversation, and survives reloads", async ({
  ...
  // d4c27099ad (#433) gave the subagent panel its own ChatInput, so while that panel is open
  // the main composer is the textarea outside the dock.
  const ta = page.locator('textarea[placeholder*="输入消息"]:not([data-testid="dock"] *)');
  const send = page.locator('button[aria-label="发送"]:not([data-testid="dock"] *)');
  await ta.waitFor();
  ...
  await expect(agentsTabActive(page)).toBeVisible();
  // d4c27099ad (#433) made the default scope pin the newest Task that actually spawned a
  // subagent ("a plain user message must not wipe the graph"), so a follow-up Task that
  // spawns nothing keeps the previous Task's graph instead of falling back to the empty
  // note — the reopen's default scope is task 1's root + child, not task 2's empty graph.
  await expect(graph.getByRole("button", { name: /General Agent/ })).toHaveCount(2);
  await expect(page.getByText("Subagent report: 3 TODOs")).toBeVisible();
