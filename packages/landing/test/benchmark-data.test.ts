...
describe("benchmark data archive (not published)", () => {
  it("formats accuracy, token, and currency values", () => {
  ...
  it("formats the second suite using the same formatters", () => {
  ...
  it("keeps every archived result explicitly marked provisional", () => {
    for (const suite of [DATA_BENCH, CODE_BENCH]) {
      expect(suite.length).toBeGreaterThan(0);
      expect(suite.every((r) => r.provisional === true)).toBe(true);
    }
  });
});
