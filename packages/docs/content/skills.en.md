...
| | `language-porting` | Port a product out of TypeScript into other languages — three skills: `port-architecture` (decide which subsystem ports to which language and where the seams go), `code-port` (carry one file or module across without changing behavior) and `port-verify` (prove the result matches the original before anyone calls it done) |
