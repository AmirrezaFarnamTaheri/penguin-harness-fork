...
The rule is about the shape of the request, not about those three examples. It fires on **any** message naming a model, a thinking level, or both, in any language — "set thinking to high", "switch to opus high", "use sonnet":

- **Level words** are `max`, `high`, `medium`/`med`, `low`, `none`/`off` and their common abbreviations. A level word sitting next to a model name is always the thinking level — never part of the model's name.
- **Model + level** ("fable5 max") → set both, model first.
- **Level only** ("set thinking to high") → change the thinking level, leave the model alone.
- **Model only** ("switch to opus") → change the model, leave the level alone.
...
| User asks for "fable5 max" / "opus high" / "set thinking to max"| model and/or thinking level — session control in the TUI (§3.4), not one unknown model, not a chat message to relay |
