...
function Test-ReleaseTag([string]$Value) {
  # `v` then a dotted numeric version, matching is_release_tag in install.sh, assertSafeTag
  # in generate-release-download-manifest.mjs and the stamp guard in release.yml. The prior
  # `^v[0-9A-Za-z]...` accepted `v1foo`, which has no release asset, so the failure surfaced
  # as a download 404 instead of an argument error.
