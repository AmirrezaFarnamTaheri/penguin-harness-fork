 ...
 *   node scripts/gen-ifaces.mjs ... --watch        # regenerate as sources change
...
import { watchIfaces } from "./gen-ifaces-watch.mjs";
...
if (
  ...
  !outPath ||
  outPath.startsWith("--") ||
  projects.some((p) => !p || p.startsWith("--")) ||
  (watch && checkOnly)
) {
  console.error(
    "usage: gen-ifaces --project <tsconfig> [--project ...] --out <ifaces.json> [--check | --watch]",
  );
  process.exit(2);
}

function generate(programs) {
  const warnings = [];
  const errors = [];
  ...
  for (const project of projects) {
    ...
    const refLiteral = (node, file) => {
      if (ts.isIdentifier(node)) {
        ...
        return {
          $id: sym && sym.flags & ts.SymbolFlags.Alias ? checker.getAliasedSymbol(sym) : sym,
        };
      }
      if (ts.isObjectLiteralExpression(node)) {
        const out = {};
    ...
    for (const [sym, { node, meta, file, kind, className }] of moduleClasses) {
      ...
      if (kind === "component")
        componentKeyBySymbol.set(
          sym,
          `${moduleNameOf(node.getSourceFile().fileName)}#${className}`,
        );
    }
    ...
    function projectData(type, atNode, stack) {
      ...
      if (f & ts.TypeFlags.Object) {
        ...
        if (checker.isArrayType(type) || name === "ReadonlyArray") {
          ...
          return typeof inner === "string" && /^[\w'|]+$/.test(inner)
            ? `${inner}[]`
            : [inner, "[]"];
        }
      ...
      if (f & ts.TypeFlags.Object) {
        ...
        if (hasMethods(type, atNode) || isIfaceClass(symbol)) {
          ...
          if (!decl)
            return fail(
              atNode,
              `anonymous object with methods; declare it as an exported interface`,
            );
    ...
    function ifaceOf(symbol, decl) {
      ...
      for (const prop of type.getProperties()) {
        ...
        if (!isMethodLike(prop, decl)) {
          ...
          const { type: kept, nullable } = optional
            ? sansUndefined(t)
            : { type: t, nullable: false };
          const projected = tolerant(prop, () =>
            exprOf(kept, prop.valueDeclaration ?? decl, [type]),
          );
    ...
    for (const [sym, { node }] of moduleClasses)
      if (componentKeyBySymbol.has(sym)) keyOf(sym, node);
